...
def copy(from_tensor: TensorLike, to_tensor: TensorLike, allow_requantize: bool = True):
    ...
    if from_type == to_type:
        ...
        assert (
            allow_requantize
            or not lib.ggml_is_quantized(from_type)
            or not lib.ggml_is_quantized(to_type)
...
def numpy(
    ...
    if lib.ggml_is_quantized(tensor.type):
        if allow_copy == False:
            raise ValueError(
                f"{__describe(tensor)} is quantized, conversion to numpy requires a copy (pass allow_copy=True; changes to the numpy array won't affect the original)."
            )
        elif isinstance(allow_copy, np.ndarray):
            __expect_same_layout(
                "source tensor", tensor, "dequantization output tensor", allow_copy
            )
        ...
        assert __is_contiguous(
            tensor
        ), f"Cannot convert {__describe(tensor)} to numpy (support contiguous tensors only)"
        nbytes = lib.ggml_nelements(tensor) * lib.ggml_type_size(tensor.type)
        array = np.frombuffer(
            ffi.buffer(lib.ggml_get_data(tensor), nbytes), dtype=dtype
        )
        array.shape = shape
        return array
...
def __type_to_dtype(type: int) -> Optional[np.dtype]:
    return __type_to_dtype_dict.get(type)


def __dtype_to_type(dtype: np.dtype):
    if dtype == np.float32:
        return lib.GGML_TYPE_F32
    elif dtype == np.float16:
        return lib.GGML_TYPE_F16
    elif dtype == np.int32:
        return lib.GGML_TYPE_I32
    elif dtype == np.int16:
        return lib.GGML_TYPE_I16
    elif dtype == np.int8:
        return lib.GGML_TYPE_I8
    else:
        raise ValueError(f"Unsupported dtype: {dtype}")


def __describe(tensor: ffi.CType):
    return f"Tensor[{__type_name(__get_type(tensor))}, {__get_shape(tensor)}]"


def __get_type(tensor: TensorLike):
    return (
        __dtype_to_type(tensor.dtype) if isinstance(tensor, np.ndarray) else tensor.type
    )


def __get_shape(x: TensorLike):
    return (
        x.shape
        if isinstance(x, np.ndarray)
        else tuple(
            [x.ne[i] for i in range(4) if x.ne[i] > 1]
        )  # // nb[0] = ggml_type_size(type)
    )


def __get_strides(x: TensorLike):
    return (
        x.strides
        if isinstance(x, np.ndarray)
        else tuple(
            [x.nb[i] for i in range(4) if x.ne[i] > 1]
        )  # // nb[0] = ggml_type_size(type)
    )


def __get_data(x: TensorLike) -> ffi.CData:
    return ffi.from_buffer(x) if isinstance(x, np.ndarray) else lib.ggml_get_data(x)


def __get_nbytes(tensor: TensorLike):
    return tensor.nbytes if isinstance(tensor, np.ndarray) else lib.ggml_nbytes(tensor)


def __get_nelements(tensor: TensorLike):
    return tensor.size if isinstance(tensor, np.ndarray) else lib.ggml_nelements(tensor)


def __is_contiguous(tensor: TensorLike):
    return (
        tensor.flags["C_CONTIGUOUS"]
        if isinstance(tensor, np.ndarray)
        else lib.ggml_is_contiguous(tensor)
    )
...
def __expect_same_layout(
    ...
    assert (
        __is_contiguous(tensor1) and __is_contiguous(tensor2)
    ), f"Only contiguous tensors are supported (got {name1} with strides {__get_strides(tensor1)} and {name2} with strides {__get_strides(tensor2)})"
...
def __check_shape_consistent_with_type(tensor: TensorLike):
    ...
    assert not (
        block_size == 0 and type in __k_quant_types
    ), f"Can't quantize, native library was not compiled with USE_K_QUANTS!"
    assert (
        block_size > 0
    ), f"Invalid block size {block_size} for type {__type_name(type)}"
    for i, d in enumerate(shape):
        assert (
            d % block_size == 0
        ), f"Dimension {i} of {__describe(tensor)} is not divisible by {block_size}, required for quantization."
