...
class PythonStubFuncDeclVisitor(c_ast.NodeVisitor):
    ...
    def get_source_snippet_lines(
        self, coord: pycparser.plyparser.Coord
        ...
        ncomment_lines = len(
            list(
                itertools.takewhile(
                    lambda i: re.search(r"^\s*(//|/\*)", source_lines[i]),
                    range(coord.line - 2, -1, -1),
                )
            )
        )
        comment_lines = [
            l.strip()
            for l in source_lines[coord.line - 1 - ncomment_lines : coord.line - 1]
        ]
        decl_lines = []
...
def generate_stubs(header: str):
    ...
    with tempfile.NamedTemporaryFile(mode="w+", suffix="_gen.h") as fh:
        fh.write(header)
        fh.flush()
        fh.seek(0)
        v.visit(CParser().parse(header, fh.name))
